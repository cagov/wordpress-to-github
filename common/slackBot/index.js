...
const slackBotChannelReplies = async (channel, ts) =>
  fetchRetry(
    `${slackApiChannelReplies}?channel=${channel}&ts=${ts}`,
    slackApiGet()
  );
...
const slackBotDelayedChatPost = async (channel, text, post_at) => {
  ...
  const fetchResp = await fetchRetry(
    "https://slack.com/api/chat.scheduleMessage",
    slackApiPost(payload)
  );
 ...
 */
const slackBotReportError = async (
  channel,
  title,
  errorObject,
  request,
  ...
  const lastHourHistory = history.messages.filter(
    c =>
      c.text.startsWith(`${title}\n`) &&
      // @ts-ignore
      // prettier-ignore
      (new Date() - new Date(1000 * Number(c.latest_reply || c.ts))) / 1000 / 60 / 60 < 1
  ); //last hour
